...
public class BloomingIndexer implements Indexer {
    ...
    public BloomingIndexer(final DecoratedKey key, final BloomingIndexSerde serde, final ColumnMetadata indexedColumn,final int nowInSec,final WriteContext ctx,
    ...
    public void updateRow(Row oldRowData, Row newRowData) {
        ...
        removeRow( oldRowData, BFUtils.getIndexKeys( oldBytes ).filterKeep( key -> {return BitMap.contains(changes, key.getPosition());} ));
        // insert any new rows that have  changed
        insertRow( newRowData, BFUtils.getIndexKeys( newBytes ).filterKeep( key -> {return BitMap.contains(changes, key.getPosition());} ));
