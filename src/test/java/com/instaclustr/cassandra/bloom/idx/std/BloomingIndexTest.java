...
public class BloomingIndexTest extends CQLTester
{
    ...
    public void testInsertAndRetrieval() throws Throwable
    {
        ...
        UntypedResultSet results = execute(String.format("SELECT * FROM %%s WHERE filter = %s", GeoName.CassandraSerde.hexString(gn[2].filter)));
        assertFalse( results.isEmpty() );
        assertEquals( 1, results.size());
        GeoName result = GeoName.CassandraSerde.deserialize( results.one() );
        assertEqual( gn[2], result );

        results = execute(String.format("SELECT * FROM %%s WHERE filter = %s", GeoName.CassandraSerde.hexString(gn[1].filter)));
        assertFalse( results.isEmpty() );
        assertEquals( 1, results.size());
        ...
        results = execute(String.format("SELECT * FROM %%s WHERE filter = %s", GeoName.CassandraSerde.hexString(gn[0].filter)));
        assertFalse( results.isEmpty() );
        assertEquals( 1, results.size());
        result = GeoName.CassandraSerde.deserialize( results.one() );
        ...
        results = execute(String.format("SELECT * FROM %%s WHERE filter = %s", GeoName.CassandraSerde.hexString(test)));
        assertFalse( results.isEmpty() );
        assertEquals( 1, results.size());
        result = GeoName.CassandraSerde.deserialize( results.one() );
        assertEqual( gn[0], result );
    ...
    public void testUpdate() throws Throwable
    {
        ...
        UntypedResultSet results = execute(String.format("SELECT * FROM %%s WHERE filter = %s", GeoName.CassandraSerde.hexString(test)));
        assertFalse( results.isEmpty() );
        assertEquals( 1, results.size());
        GeoName result = GeoName.CassandraSerde.deserialize( results.one() );
    ...
    public void testDelete() throws Throwable
    {
        ...
        UntypedResultSet results =  execute(String.format("SELECT * FROM %%s WHERE filter = %s", GeoName.CassandraSerde.hexString(gn[1].filter)));
