...
import com.instaclustr.cassandra.bloom.idx.IdxTable;
...
//    private IdxTable idxTable;
    ...
    private static final String index = "CREATE CUSTOM INDEX ON geonames.geoname(bf) USING 'com.instaclustr.cassandra.bloom.idx.std.BloomingIndex'";

    public BloomingIndexTest() {
        CustomIndexTest t = new CustomIndexTest();
...
//    }
   ...
   public void testInsertsOnCfsBackedIndex() throws Throwable
   {
       ...
       execute("INSERT INTO %s (a, b, c, d) VALUES (?, ?, ?, ?)", 0, 0, 0, bufferFromBloom(gn[0].filter));
       execute("INSERT INTO %s (a, b, c, d) VALUES (?, ?, ?, ?)", 0, 1, 0, bufferFromBloom(gn[1].filter));
       execute("INSERT INTO %s (a, b, c, d) VALUES (?, ?, ?, ?)", 0, 2, 0, bufferFromBloom(gn[2].filter));
   }

   private static ByteBuffer bufferFromBloom( BloomFilter filter ) {
       ByteBuffer bb = ByteBuffer.allocate( BitMap.numberOfBitMaps( filter.getShape().getNumberOfBits())*Long.BYTES );
       filter.forEachBitMap( bb::putLong );
       return bb;
   }
